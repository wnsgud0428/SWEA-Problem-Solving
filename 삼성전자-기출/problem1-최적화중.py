...
def returnLowestRow_optimize(arr, dp, golem: Golem):
    for r in range(len(arr) - 1, 3, -1):
        if True in arr[r]:
            dp[golem.num] = r
            return r
...
for golem in golems:
    ...
    # 다 내려온 후, golem 정보에 맞게 board.info 수정
    board.info[golem.r][golem.c] = golem.num  # 골렘 중앙
    for i in range(4):
        nr = golem.r + dr2[i]
        nc = golem.c + dc2[i]
    ...
    def 정령이_갈수있는_visited배열_구하고_lowest_row_return():
        max_row = 0
            ...
            for i in range(4):
                ...
                if 0 <= nr <= len(board.info) - 1 and 0 <= nc <= len(board.info[0]) - 1:
                    if board.info[nr][nc] != 0 and board.info[nr][nc] != v and not visited[nr][nc]:
                        if board.info[nr][nc] in dp:  # 최적화 진행중..
                            max_row = max(dp[board.info[nr][nc]], max_row)
                        else:
                            q.append(board.info[nr][nc])

        if max_row != 0:
            return max_row
        return returnLowestRow_optimize(visited, dp, golem)


    most_down_row = 정령이_갈수있는_visited배열_구하고_lowest_row_return() - 3 + 1
